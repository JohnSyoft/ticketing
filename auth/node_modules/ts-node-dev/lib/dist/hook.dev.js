...
exports.makeHook = function (cfg, wrapper, callback) {
  ...
  updateHooks(); // Patch the vm module to watch files executed via one of these methods:
  ...
  function createHook(handler) {
    return function nodeDevHook(module, filename) {
      ...
      if (!module.loaded) callback(module.filename); // Invoke the original handler

      handler(module, filename); // Make sure the module did not hijack the handler
