...
var fixPath = function fixPath(p) {
...
exports.makeCompiler = function (options, _a) {
  ...
  var tmpDir = options['cache-directory'] ? path_1["default"].resolve(options['cache-directory']) : fs_1["default"].mkdtempSync(path_1["default"].join(os_1["default"].tmpdir(), '.ts-node'));

  var writeChildHookFile = function writeChildHookFile(options) {
    var compileTimeout = parseInt(options['compile-timeout']);

    var getIgnoreVal = function getIgnoreVal(ignore) {
      var ignoreVal = !ignore || ignore === 'false' ? 'false' : '[' + ignore.split(/,/).map(function (i) {
        return i.trim();
      }).map(function (ignore) {
    ...
    var varDecl = function varDecl(name, value) {
      return "var " + name + " = '" + value + "'";
    };

    var replacements = [compileTimeout ? ['10000', compileTimeout.toString()] : null, allowJs ? ['allowJs = false', 'allowJs = true'] : null, options['prefer-ts-exts'] ? ['preferTs = false', 'preferTs = true'] : null, options['exec-check'] ? ['execCheck = false', 'execCheck = true'] : null, options['exit-child'] ? ['exitChild = false', 'exitChild = true'] : null, options['ignore'] !== undefined ? ['var ignore = [/node_modules/]', 'var ignore = ' + getIgnoreVal(options['ignore'])] : null, [varDecl('compilationId', ''), varDecl('compilationId', getCompilationId())], [varDecl('compiledDir', ''), varDecl('compiledDir', getCompiledDir())], ['./get-compiled-path', fixPath(path_1["default"].join(__dirname, 'get-compiled-path'))], [varDecl('readyFile', ''), varDecl('readyFile', getCompilerReadyFilePath())], [varDecl('sourceMapSupportPath', ''), varDecl('sourceMapSupportPath', fixPath(sourceMapSupportPath))], [varDecl('libPath', ''), varDecl('libPath', __dirname.replace(/\\/g, '\\\\'))], ['__dirname', '"' + fixPath(__dirname) + '"']].filter(function (_) {
      return !!_;
    }).map(function (_) {
      return _;
    });
  ...
  var init = function init() {
    ...
    createCompiledDir(); // check if `allowJs` compiler option enable
  ...
  var getCompilationId = function getCompilationId() {
    return compilationInstanceStamp;
  };

  var createCompiledDir = function createCompiledDir() {
  ...
  var getCompiledDir = function getCompiledDir() {
    return path_1["default"].join(tmpDir, 'compiled').replace(/\\/g, '/');
  };

  var getCompileReqFilePath = function getCompileReqFilePath() {
    return path_1["default"].join(getCompiledDir(), getCompilationId() + '.req');
  };

  var getCompilerReadyFilePath = function getCompilerReadyFilePath() {
    return path_1["default"].join(os_1["default"].tmpdir(), 'ts-node-dev-ready-' + compilationInstanceStamp).replace(/\\/g, '/');
  };

  var getChildHookPath = function getChildHookPath() {
    return path_1["default"].join(os_1["default"].tmpdir(), 'ts-node-dev-hook-' + compilationInstanceStamp + '.js').replace(/\\/g, '/');
  };

  var writeReadyFile = function writeReadyFile() {
    fs_1["default"].writeFileSync(getCompilerReadyFilePath(), '');
  };

  var clearErrorCompile = function clearErrorCompile() {
    clearTimeout(_errorCompileTimeout);
  };

  var registerTsNode = function registerTsNode() {
    ...
    var scriptPath = options._.length ? path_1["default"].resolve(cwd, options._[0]) : undefined;
    tsNode.register({
      ...
      ignoreDiagnostics: options['ignore-diagnostics'] ? split(options['ignore-diagnostics']) : undefined,
  ...
  var compiler = {
    ...
    compileChanged: function compileChanged(fileName) {
    ...
    compile: function compile(params) {
      ...
      var code = fs_1["default"].readFileSync(fileName, 'utf-8');
      var compiledPath = params.compiledPath; // Prevent occasional duplicate compilation requests
      ...
      try {
        ...
        writeCompiled(errorCode); // reinitialize ts-node compilation to clean up state after error
        ...
        var timeoutMs_1 = parseInt(process.env.TS_NODE_DEV_ERROR_RECOMPILE_TIMEOUT || '0') || 5000;

        var errorHandler_1 = function errorHandler_1() {
          clearTimeout(_errorCompileTimeout);
