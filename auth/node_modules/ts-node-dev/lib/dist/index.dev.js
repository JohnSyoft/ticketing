...
exports.runDev = function (script, scriptArgs, nodeArgs, opts) {
  ...
  var notify = notify_1.makeNotify(cfg, log); // Run ./dedupe.js as preload script
  ...
  var starting = false; // Read for "rs" from command line
  ...
  log.info('ts-node-dev ver. ' + version + ' (using ts-node ver. ' + tsNodeVersion + ', typescript ver. ' + tsVersion + ')');
  /**
  ...
  function start() {
    if (cfg.clear) process.stdout.write("\x1Bc");
    ...
    ipc.on(child, 'required', function (m) {
      ...
      var isIgnored = cfg.ignore.some(isPrefixOf(required)) || cfg.ignore.some(isRegExpMatch(required));
  ...
  function restart(file, isManualRestart) {
    ...
    log.debug('Removing all watchers from files'); //watcher.removeAll()ya
