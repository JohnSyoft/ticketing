...
var tsNodeFlags = {
  "boolean": ['scope', 'emit', 'files', 'pretty', 'transpile-only', 'prefer-ts-exts', 'prefer-ts', 'log-error', 'skip-project', 'skip-ignore', 'compiler-host', 'script-mode'],
  string: ['compiler', 'project', 'ignore', 'ignore-diagnostics', 'compiler-options', 'scopeDir', 'transpiler']
};
var tsNodeAlias = {
  'transpile-only': 'T',
  'compiler-host': 'H',
...
var devFlags = {
  "boolean": ['deps', 'all-deps', 'dedupe', 'fork', 'exec-check', 'debug', 'poll', 'respawn', 'notify', 'tree-kill', 'clear', 'cls', 'exit-child', 'error-recompile', 'quiet', 'rs'],
  string: ['dir', 'deps-level', 'compile-timeout', 'ignore-watch', 'interval', 'debounce', 'watch', 'cache-directory']
};
var opts = minimist_1["default"](devArgs, {
  ...
  alias: __assign(__assign({}, tsNodeAlias), {
...
_unknown.forEach(function (arg) {
  if (arg === script || nodeArgs.indexOf(arg) >= 0) return;
  var argName = arg.replace(/^-+/, ''); // fix this
...
if (!script) {
  ...
  console.log('ts-node-dev: no script to run provided'); // eslint-disable-next-line no-console
