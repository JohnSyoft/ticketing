...
var waitForFile = function waitForFile(fileName) {
  var start = new Date().getTime();

  while (true) {
    var exists = execCheck ? execSync(['node', checkFileScript, '"' + fileName + '"'].join(' '), {
      stdio: 'inherit'
...
var sendFsCompileRequest = function sendFsCompileRequest(fileName, compiledPath) {
...
var compile = function compile(code, fileName) {
...
function registerExtensions(extensions) {
  ...
  if (preferTs) {
    var reorderRequireExtension_1 = function reorderRequireExtension_1(ext) {
      var old = require.extensions[ext];
      delete require.extensions[ext];
      require.extensions[ext] = old;
    };

    var order = ['.ts', '.tsx'].concat(Object.keys(require.extensions).filter(function (_) {
      return _ !== '.ts' && _ !== '.tsx';
    }));
...
function registerJsExtension() {
  var old = require.extensions['.js']; // handling preferTs probably redundant after reordering
