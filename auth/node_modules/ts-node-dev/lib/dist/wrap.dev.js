...
var cfg_1 = require("./cfg"); // const Module = require('module')
// Remove wrap.js from the argv array


process.argv.splice(1, 1); // Resolve the location of the main script relative to cwd
...
if (cfg.fork) {
  var oldFork_1 = child_process_1.fork; // Overwrite child_process.fork() so that we can hook into forked processes
  // too. We also need to relay messages about required files to the parent.

  var newFork = function newFork(modulePath, args, options) {
...
process.on('uncaughtException', function (err) {
  ...
  if (caught) return;
  caught = true; // If there's a custom uncaughtException handler expect it to terminate
  // the process.
